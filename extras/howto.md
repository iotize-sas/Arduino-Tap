...
You also need to declare and initialize the Tap handler. This is done by adding the line:

        Tap  myTap(pin_clk, pin_io); 
...
If the library is correctly included in your project, the Arduino file should compile without problem after adding the tap declaration. 
...
Note that you need the ELF file to make IoTize Studio to generate the S3P_conf.h... needed to generate the ELF file.  If you have entered this loop, you can break the loop by commenting out the Tap declaration in order to generate the ELF file then to do the synchronization process.  
...
The possible known issues are: 
 ...
 - with your change: check that you declare  'Tap myTap(CLK_PIN, DATA_PIN);' with the right pin numbers,
